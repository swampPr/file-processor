...
async function compressDefault(file: Buffer, id: SessionID) {
    try {
        ...
        const proc = Bun.spawn(['gs', '-sDEVICE=pdfwrite', '-dCompatibilityLevel=1.4', '-dPDFSETTINGS=/ebook', '-dNOPAUSE', '-dQUIET', '-dBATCH', '-dNOGC', '-dNumRenderingThreads=4', '-sOutputFile=output.pdf', '-c', '"30000000', 'setvmthreshold"', '-f', 'input.pdf'], {
...
async function compressAggressive(file: Buffer, id: SessionID) {
    try {
        await Bun.write(`./src/backend/sessions/${id}/input.pdf`, file);
        //NOTE: After this handle the actual compressing using pdfcpu
        const proc = Bun.spawn(['gs', '-sDEVICE=pdfwrite', '-dCompatibilityLevel=1.4', '-dPDFSETTINGS=/screen', '-dColorConversionStrategy=/Gray', '-dProcessColorModel=/DeviceGray', '-dColorImageResolution=72', '-dGrayImageResolution=72', '-dMonoImageResolution=72', '-dDownsampleColorImages=true', '-dDownsampleGrayImages=true', '-dDownsampleMonoImages=true', '-dCompressFonts=true', '-dSubsetFonts=true', '-dNOGC', '-dNumRenderingThreads=4', '-dNOPAUSE', '-dQUIET', '-dBATCH', '-sOutputFile=output.pdf', '-c', '30000000', 'setvmthreshold', '-f', 'input.pdf'], {
            cwd: `./src/backend/sessions/${id}`,
        });
        await proc.exited;
        const outputFile = Bun.file(`./src/backend/sessions/${id}/output.pdf`);
